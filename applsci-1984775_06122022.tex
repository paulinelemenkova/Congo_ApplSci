...
\Author{\hl{Polina Lemenkova} %MDPI: Please carefully check the accuracy of names and affiliations.
 \hl{*} %MDPI: Newly added * for Correspondence author, please confirm
...
\address[1]{%
Laboratory of Image Synthesis and Analysis, École Polytechnique de Bruxelles (EPB, Brussels Faculty of Engineering), Université Libre de \hl{Bruxelles,} %MDPI: The provided information should be arranged from subordinate to superior, we revised aff order, please confirm
...
\PACS{\hl{91.10.Da;} %MDPI: For this journal, these ``PACS/MSC/JEL Classification'' parts are not necessary, please confirm if these parts should be retained.
...
\featuredapplication{Libraries of the R programming language, including \emph{RStoolbox}, \emph{raster} and \emph{terra}, were used to classify Landsat OLI-TIRS satellite images by \emph{k}-means clustering and NDVI computation for comparative vegetation mapping of tropical rainforests for the period of 2013-2022 in the three target areas (Bumba, Basoko, Kisangani) of the middle Congo River Basin, central Africa, D.R.C.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\end{adjustwidth}
	\caption{\hl{Composite} %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
...
The generated images reveal spatial details that can be better discerned in the intensity of pixels in false colour composites for water areas and more sharpened representations of urban areas in the natural colour composites showing spatial details in the three target areas: Bumba, Basoko and Kisangani (Figure~\ref{fig:05}). The colour products for all the three regions were prepared using the combination of bands 4-3-2 (natural) and 5-4-3 (false) with the spatial resolution of the visual bands selected as three triplets of the original multispectral bands of the Landsat-8 OLI/TIRS. The new mixture from the RGB bands was converted to the colours with the intensity channel corresponding to the resolution of the original bands (30~m). Hue, saturation and intensity transformations of each of the triplets were processed and resampled by R package 'terra' to form the output image, as seen in Figure~\ref{fig:05}.
...
\end{adjustwidth}
\caption{Natural (bands 4-3-2) and false colour (bands 5-4-3) composites of the Landsat 8 OLI/TIRS. \label{fig:05}}
%image for the three target areas: Bumba, Basoko and Kisangani. Mapping: RStudio. (\textbf{a}) Bumba: natural colour composite, 2013. (\textbf{b}) Bumba: false colour composite: 2013. (\textbf{c}) Basoko: natural colour composite: 2013. (\textbf{d}) Basoko: false colour composite: 2013. (\textbf{e}) Kisangani: Natural colour composite: 2013. (\textbf{f}) Kisangani: False colour \hl{composite: 2013.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
%%}
\end{figure}

\begin{figure*}
\captionof*{figure}{Natural (bands 4-3-2) and false colour (bands 5-4-3) composites of the Landsat 8 OLI/TIRS image for the three target areas: Bumba, Basoko and Kisangani. Mapping: RStudio. (\textbf{a}) Bumba: natural colour composite, 2013. (\textbf{b}) Bumba: false colour composite: 2013. (\textbf{c}) Basoko: natural colour composite: 2013. (\textbf{d}) Basoko: false colour composite: 2013. (\textbf{e}) Kisangani: Natural colour composite: 2013. (\textbf{f})~Kisangani: False colour \hl{composite: 2013.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
}
\end{figure*}
...
The \hl{\emph{k}-means} %MDPI: Please confirm if k should be normal foramt in "k-means", and unify the writing of the full text
...
\end{adjustwidth}
	\caption{Executing the \emph{k}-means clustering classification algorithm in RStudio using the RStoolbox package. Parameters of cluster centroids and sum of squares by cluster are displayed in the console of R. Spatial parameters of the output map are listed. Here is shown the example of Landsat image of~Basoko.
	\label{fig:06}}
\end{figure} 
...
\caption{\hl{Bumba} %MDPI: We moved Figures 7--9 AFTER where it is first mentioned in the text.
 town and surroundings of the Congo River: raw Landsat-8 OLI/TIRS scene for years 2013 and 2022 and the classified outputs by \emph{k}-means clustering. Mapping: RStudio. Source: authors. (\textbf{a}) Monochrome image for Bumba region: 2013. (\textbf{b}) Classified scene using \emph{k}-means clustering: 2013. (\textbf{c}) Monochrome image for Bumba region: 2022. (\textbf{d}) Classified scene using \emph{k}-means \hl{clustering: 2022.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
}\label{fig:07}
\end{figure}



The unsupervised learning algorithm of R incorporates the parameter estimation for pixels under a partitioning framework using cell's brightness, which corresponds to the spectral reflectance of the objects in a matrix of the raster scene. The algorithm of \emph{k}-means clustering is computationally demanding by R, yet it is crucial to the concept of remote sensing due to the effectiveness and importance of the output in satellite image processing. Thus, land cover types are not known beforehand and defined by the machine method of image analysis based on the spectral signatures of the objects partitioned into the series of clusters. The data were divided automatically into the ten clusters using the search for the spectral class for each pixel in the image. 
...
\caption{Basoko town (Tshopo Province) and surroundings in the confluence of the Aruwimi River tributary into the main stream of the Congo River: raw Landsat-8 OLI/TIRS scene for years 2013 and 2022 and the classified outputs by \emph{k}-means clustering. Mapping: RStudio. Source: authors. (\textbf{a})~{True-colour composite }image for Basoko: 2013. (\textbf{b}) Classified scene using \emph{k}-means clustering: 2013. (\textbf{c}) {True-colour composite }image for Basoko: 2022. (\textbf{d}) Classified scene using \emph{k}-means \hl{clustering: 2022.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
}\label{fig:08}
\end{figure}
...
\caption{Kisangani city (Tshopo Province) and region in the junction of the Tshopo and Lindi tributaries into the Congo River: raw Landsat-8 OLI/TIRS scene for years 2013, 2015 and 2022 and the classified outputs by \emph{k}-means clustering. Mapping: RStudio. Source: authors. (\textbf{a}) {True-colour composite }image, Kisangani, 2013. (\textbf{b}) Classified scene using \emph{k}-means clustering: 2013. (\textbf{c}) Classified scene using \emph{k}-means clustering: 2015. (\textbf{d}) Classified scene using \emph{k}-means \hl{clustering: 2022.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
}\label{fig:09}
\end{figure}
...
\caption{\hl{NDVI} %MDPI: We moved this figure AFTER where it is first mentioned in the text.
 computed from 5 and 4 bands of the Landsat-8 OLI/TIRS scenes for years 2013 and 2022 for the three key areas: Bumba, Basoko, Kisangani. Mapping: RStudio. Source: authors. (\textbf{a}) Bumba region: 2013. (\textbf{b}) Bumba region: 2022. (\textbf{c}) Basoko region: 2013. (\textbf{d}) Basoko region: 2022. (\textbf{e}) Kisangani region: 2013. (\textbf{f}) Kisangani \hl{region: 2022.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
}\label{fig:10}
\end{figure}
...
\funding{This research was funded by the Editorial Office of the Applied Sciences, Multidisciplinary Digital Publishing Institute (MDPI), by providing a 100\% discount for APC for the publication of this~manuscript.}
...
\newblock A vegetation map of the Central Congo basin derived from microwave
  ...
  approach.
\newblock In Proceedings of the Geoscience and Remote Sensing Symposium, 2001. IGARSS '01, \hl{Sydney, NSW, Australia, 9--13 July 2001;} %MDPI: We added the location and date of the conference. Please confirm.
...
\newblock In Proceedings of the {IGARSS 2003. 2003 IEEE International
  ...
  No.03CH37477)},  \hl{Toulouse, France, 21--25 July 2003;} %MDPI: We added the location and date of the conference. Please confirm.
...
\newblock In Proceedings of the IGARSS 2000. IEEE 2000 International Geoscience
  ...
  Remote Sensing in Managing the Environment. Proceedings (Cat. No.00CH37120), \hl{Honolulu, HI, USA, 24--28 July 2000;} %MDPI: We added the location and date of the conference. Please confirm.
...
\newblock Selective logging of tropical forests observed using L- and C-band
  SAR satellite data.
\newblock In Proceedings of the 2015 IEEE International Geoscience and Remote
  Sensing Symposium (IGARSS),  \hl{Milan, Italy, 26--31 July 2015;} %MDPI: We added the location and date of the conference. Please confirm.
   pp. 3870--3873.
...
\newblock Forest monitoring in Congo Basin with combined use of SAR C- \&
  L-band.
\newblock In Proceedings of the 2012 IEEE International Geoscience and Remote
  Sensing Symposium, \hl{Munich, Germany, 22--27 July  2012;} %MDPI: We added the location and date of the conference. Please confirm.
 pp. 6573--6576.
...
\newblock In Proceedings of the 2017 9th International Workshop on the Analysis
  of Multitemporal Remote Sensing Images (MultiTemp), \hl{Brugge, Belgium, 27--29 June 2017;} %MDPI: We added the location and date of the conference. Please confirm.
 pp. 1--4.
...
\newblock \textls[-15]{In Proceedings of the 2011 19th International Conference on
  Geoinformatics, \hl{Shanghai, China,  24--26 June 2011;} %MDPI: We added the location and date of the conference. Please confirm.
...
\newblock {\em Les Forêts du Bassin du Congo---Etat des Forêts 2008}; Office
  des Publications de L\'union Européenne:  \hl{Luxembourg,} %MDPI: We added the location of the publisher. Please confirm.
 2009.
...
\newblock {\em Package ‘RStoolbox’};  \hl{2022.} %MDPI: Please add the name of the publisher and their location.
...
\newblock \hl{Maintainer: Vienna, Austria,} %MDPI: Please confirm if  publisher and their location is correct?
  2022.
...
Neuwirth, E.
\newblock RColorBrewer: ColorBrewer Palettes. R Package Version 1.1-2.  2014. Available online: \url{https://CRAN.R-project.org/package=RColorBrewer}  \hl{(accessed on).} %MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
...
\newblock \hl{R Package Version 0.6.2.2021.} %MDPI: Please add the name of the publisher and their location.
...
\newblock In Proceedings of the IGARSS 2008---2008 IEEE International
  Geoscience and Remote Sensing Symposium, \hl{Boston, MA, USA,  7--11 July 2008;} %MDPI: We added the location and date of the conference. Please confirm.
 Volume~5, pp. 491--494.
...
\newblock Online,  2020.
\newblock LSDS-1822 Version 6.0, USGS.  Available online: \url{https://earth.esa.int/eogateway/documents/20142/0/Landsat-8-9-OLI-TIRS-Collection-2-Level-1-Data-Format-Control-Book-DFCB.pdf}  \hl{(accessed on).} %MDPI: Newly added URL for this ref, please confirm. Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
...
\newblock \hl{https://doi.org/10.1007/s11273-012-9277-z.} %MDPI: only the editorial type papers can retain the "Short Biography of Author" in the manuscript, other types of papers are not allowed Keep it, so we remove this part, please confirm

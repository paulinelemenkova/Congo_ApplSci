...
\abstract{In this paper, \textcolor{blue}{an image analysis framework is formulated for Landsat-8 OLI-TIRS scenes using R programming language. The libraries of R were shown to be effective in remote sensing data processing tasks such as} classification using \textcolor{blue}{k}-means clustering \textcolor{blue}{ and }computing the Normalized Difference Vegetation Index (NDVI). \textcolor{blue}{The} data handling is processed using \textcolor{blue}{the} integration of RStoolbox, terra, raster, rgdal \textcolor{blue}{and auxiliary} packages \textcolor{blue}{of R}. \textcolor{blue}{The} proposed \textcolor{blue}{approach} of image processing using R is designed to exploit \textcolor{blue}{the} parameters of image bands as cues to detect land cover types and vegetation parameters corresponding to the spectral reflectance of the objects represented on the Earth's surface. Our method is effective for processing \textcolor{blue}{the} time-series of the images taken in various periods to monitor \textcolor{blue}{the} landscape dynamics in \textcolor{blue}{middle part} of Congo River basin\textcolor{blue}{, D.R.C}. \textcolor{blue}{Whereas previous approaches primarily used GIS software, we proposed to explicitly use the scripting methods for satellite image analysis by applying the extended functionality of R}. \textcolor{blue}{The application} of scripts \textcolor{blue}{for} geospatial data is effective and robust method contrasting to \textcolor{blue}{the traditional} approaches due to \textcolor{blue}{the} high automation and machine-based graphical \textcolor{blue}{processing}. \textcolor{blue}{The} algorithms of R libraries are adjusted to spatial operations \textcolor{blue}{such} as projection and transformation, \textcolor{blue}{object} topology\textcolor{blue}{, classification} and \textcolor{blue}{map algebra. The} data \textcolor{blue}{included} Landsat-8 OLI-TIRS covering \textcolor{blue}{the three regions} along the river of \textcolor{blue}{Congo:} Bumba, Basoko and Kisangani for the years 2013, 2015 and 2022\textcolor{blue}{. We} also \textcolor{blue}{validated} the performance of graphical data handling for cartographic visualization using R libraries \textcolor{blue}{for visualising changes in land cover types by k-means clustering and calculation of the NDVI for vegetation analysis.}
}
% Keywords
...
We \hl{explored} the 'rgdal' library of R to retrieve the coordinates, check the topology and spatial dimension of the dataset stored in the SpatialGridDataFrame class. \hl{A topology refers to the arrangement of the images that defines the coincident geometry of the feature classes from the Landsat dataset. It reads the contextual information from the structure data based on spatial relationships: the adjacency of the coordinates and connectivity of overlapping images}. The RasterLayers were generated as \hl{the} arrays of pixels from the individual\hl{ }bands of the Landsat scenes. Afterwards, the facetted stack was created \hl{using} the Landsat OLI-TIRS bands \hl{by} stack() function\hl{. To this end, the 'raster' R library was applied for} concatenating \hl{the} multiple bands into a single bitmap image which uses the data on \hl{the origin of} the coordinates of each band. The resulting image was visualized by \hl{the} plot() function of R. \hl{The} number of layers and spatial dimension of in the new multi-layer object of R \hl{were inspected by 'raster' library}. The resolution of all the layers was 30 m except for the panchromatic band (15 m) and the two last TIRS layers with \hl{a coarser} resolution \hl{100 m}. The selected band was plotted in \hl{a grayscale} monochrome visualization. The data analysis and inspection has been performed using Listing \ref{lst:01}.

\subsection{Plotting band composites}

\hl{We explored} the structure of Landsat bands \hl{by inspecting the metadata}. The image of all the bands is visualized as a representation of the monochrome multi-facetted plot \hl{in} Figure \ref{fig02}. The most \hl{of the}multispectral bands have 30-m resolution, while \hl{the} panchromatic band (B8) has a higher resolution (15 m) and the two TIRS (B10 and B11) have a coarse 100-m resolution. \hl{Generating} colour composites by combination of various band triplets \hl{provides more information which can be effectively exploited} using libraries 'raster' and 'terra' of R. Here the\hl{ }idea of the machine-assisted classifier by R packages consists in the identification of spectral reflectance of pixels constituting \hl{the }11 bands for thematic mapping. 

The \hl{sensor} instruments of Landsat-8 OLI-TIRS record more channels of data than are actually needed for vegetation applications. For instance, for NDVI calculation we only need the Red and NIR bands (4 and 5, respectively). \hl{However, combining the bands in various RGB triplets enables to highlight diverse objects on the surface, to discriminate and capture information on a wider variety of objects.} \hl{Thus, the} composition of\hl{ }red, green and blue colours creates the images with varied hue, saturation and intensity representation owing to the pixel brightness as spectral reflectance of the objects and surfaces on the Earth. Therefore,\hl{ }the bands \hl{were mixed }to form various colour composites and inspect the land cover types. The selected and most representative band composites for the three \hl{main} studied regions were plotted as natural and false colour composites, \hl{respectively}, which were generated\hl{ using }library 'raster'. The \hl{relevant }code snippet is given for the town of Basoko, and \hl{was} applied similarly for the rest of the \hl{Landsat-8 OLI-TIRS} scenes \hl{with changed parameters}, Listing \ref{lst:02}. 

\begin{figure}[H]
\makebox[\textwidth][r]{\includegraphics[width=1.1\textwidth]{Fig_02.jpg}}%
	\caption{Composite of monochrome 11 bands: Landsat-\hl{8} OLI/TIRS C1 for Basoko region (Arowumi and Congo \hl{rivers}). Multi-spectral image 'LC81770592022039LGN00': Ultra Blue (B1), Blue (B2), Green (B3), Red (B4), Near Infrared (NIR) (B5), Shortwave Infrared (SWIR) 1 (B6), Shortwave Infrared (SWIR) 2 (B7), Panchromatic (B8), Cirrus (B9), Thermal Infrared (TIRS) 1 (B10), TIRS2 (B11). Plotting: R.}
	\label{fig02}
\end{figure} 
...
The generated images reveal spatial details which can be better discerned in \hl{the} intensity of pixels in false colour composites for water areas and more sharpened representation of urban areas in the natural colour composites showing\hl{ }spatial details\hl{ }in the three \hl{target areas}: Bumba, Basoko and Kisangani, Figure \ref{fig03}. The colour products for all the three \hl{regions} were prepared using the combination of bands \hl{4-3-2} (natural) and 5-4-3 (false) with the spatial resolution of the visual bands selected as three triplets of the original multispectral bands of the Landsat-8 OLI/TIRS. The new mixture from the RGB bands is converted to the colours with the intensity channel corresponding to the resolution of the original bands (30 m). A hue, saturation and intensity transformation of each of the triplets are processed and resampled by R package 'terra' to form the output image, Figure \ref{fig03}.

\hl{The demonstrated above scripts show the key advantage of R which consists in the optimization of the workflow of remote sensing data processing. Moreover, it illustrates its advanced analysis capabilities aimed at processing raster data by scripting. Scripts can be re-used with modified lines of code for the next images as a consecutive work, rather than repeated as a whole process of image analysis for each image, as in GIS, which is a fundamentally different approach compared to GIS. This is especially actual for processing big Earth observation data which play a key role in geoscience. Rapid development of remote sensing technologies resulted in the increased amount of available satellite images, which requires advanced technologies for processing these large massifs of spatial data. In contrast to the traditional software, programming languages provide an effective solution to handle such volumes of spatial data by scripting. R, as an example of the high-level programming language, effectively solves the problem of optimization of image processing.}
...
The \emph{k}-means clustering of R package RasterStack is a method by which pixels are assigned to spectral classes by machine-based partition without prior knowledge of those classes. It is a straightforward way to produce a map of land cover classes using automatic approach \hl{of image classification}. The algorithm has been performed using \hl{image-partitioning algorithm that infers the labels of the land cover classes by analysing intra-class similarity and, vice versa, the contrast between the classes. This method} presents a machine learning approach for \hl{assigning} pixels\hl{ }into the defined groups (or coherent clusters)\hl{, }according to their intensity and brightness, Listing \ref{lst:04}. \hl{Clustering and classification of data} is a commonly accepted practice for land cover mapping \hl{aimed at }modelling changes in \hl{a} forest structure by examining land-use categories  \cite{Manakos,WILKIE1988307,4780136,rs11242999,land11060787}.
...
The image for classification was generated using stacking the data which enabled to create \hl{the} RasterStack as a collection of RasterLayer objects\hl{. In turn, they were }generated from \hl{the} raster layers of \hl{the} selected bands with the same spatial extent and resolution\hl{, that is, }30 m for the most \hl{of the} Landsat-8 OLI/TIRS bands\hl{. }Afterwards,\hl{ }RasterStack was merged into the RasterBrick, which is a multi-layer raster object created from the multi-layer files \hl{of} RasterStack\hl{, }made up of Landsat \hl{bands}. The similarity between \hl{the} RasterStack and RasterBrick classes \hl{consists in the fact} that\hl{ }both are generated with a stack of the raster Landsat bands. However, the processing time for the RasterBrick is shorter in R environment\hl{, }compared to the RasterStack, \hl{because} RasterBrick is less flexible and is merged already as a single file. The procedure and the output of the \emph{k}-means clustering is presented in Figure \ref{fig04}\hl{, }which shows the console of the RStudio with statistical information on cluster centroids and sum of squares for each processed map\hl{, respectively. }The cluster map on the right of the print screen depicts the class memberships of the pixels, in this case \hl{we give the} example of \hl{the} Basoko town \hl{on the year }2013, Figure \ref{fig04}. 
...
Thus, the automatic clustering analysis \hl{by R} has been performed without prior knowing of class membership of those pixels \hl{using a principle of image segmentation by partitioning raster file based on the generalization of the matrix of cells}. Based on the automatic analysis of pixel brightness, the pixels of a given symbol were partitioned into the same spectral class which belongs to one of the 10 clusters. \hl{The advantage of this approach is that it works fast and straightforward in operating complex scenes} The clusters were identified by associating the pixels from the images with the available information of spectral reflectance and presented as the output maps in Figure \ref{fig05} (Bumba), Figure \ref{fig06} (Basoko) and Figure \ref{fig07} (Kisangani). 

\hl{The unsupervised learning algorithm of R incorporates the parameter estimation for pixels under partitioning framework using cell's brightness that corresponds to the spectral reflectance of the objects on a matrix of the raster scene.} The algorithm of \emph{k}-means clustering is computationally demanding by R, yet it is crucial to the concept of remote sensing\hl{ }due to the effectiveness and importance of the output\hl{ in satellite image processing}. Thus, land cover types are not known beforehand and defined by the machine method of image analysis\hl{, }based on the spectral signatures of the objects partitioned into the series of clusters. The data were divided automatically into the ten clusters using the search for spectral class for each pixel in the image. 
\hl{Using }this approach, we defined 10 classes \hl{in a process of} the unsupervised classification approach to produce the thematic maps \hl{with discriminated} vegetation and land cover classes using the existing classification adopted from previous studies \cite{Bwangoy}: swamp forest; humid tropical forest; secondary forest; urban areas and impervious surfaces; inundated grassland; deciduous forest; shrubland and grassland; cropland and mosaic forest; savannah with sparse trees; water bodies. 

\subsection{NDVI calculation}
\hl{The NDVI is a key vegetation indicators to describe vegetation dynamics and vigour. It is effectively applied to environmental management and monitoring the responses of tropical rainforests to climate change and anthropogenic activities. Therefore, the use of the NDVI is crucial to reveal the health of the terrestrial ecosystems in central Africa which are threatened by the deforestation. As a robust traditional estimation method to determine vegetation coverage, NDVI is deservedly applied in a variety of the remote sensing studies due to its effectiveness and reliability. Besides, the visualization of the NDVI is indispensable in areas where fieldwork and ground points collections is hardly accessible, such as tropical African regions of central Congo. Therefore}, we adapted the algorithm of NDVI for extracting the information on vegetation for knowledge on plant coverage in the three target areas \hl{in} Congo \hl{Basin}. The NDVI has been generated using composite band arithmetic operations \hl{by} ratio of\hl{ }difference and \hl{the} sum of \hl{the} reflected NIR and visible \hl{Red} wavelength. The \hl{NDVI} approach\hl{ }is grounded on the calculation of ratios in the reflectance between \hl{the} Red and NIR bands of the image (Band 4 and 5), as arithmetical variables using \hl{the} existing formula $NDVI=(NIR-Red)/(NIR+Red)$. The NDVI computation has been performed using\hl{ }multi-band manipulation \hl{by} R library terra, Listing \ref{lst:05}. 
...
Calculation of the NDVI enabled to exploit the \hl{information derived} from\hl{ }spectral reflectance regarding vegetation coverage\hl{. This is enabled due to the parameters of the} ratios of NIR/Red spectral bands \hl{which} lessen the influence from the ground relief, rocks and soils. Thus, the methodological advantage of the NDVI consists in the reinforcement of the slight differences and nuances in\hl{ }spectral reflectance characteristics of vegetation. This enables to better distinct \hl{forest} covered areas\hl{, discriminate various vegetation classes} and discern them from all other land cover types. Using pairwise representations of the 9-year time series of Landsat OLI/TIRS (2013 to 2022), we calculated the NDVI\hl{ }for the three target regions along the Congo river: Bumba, Basoko and Kisangani. Here we computed the values of NDVI for each couple of images, respectively, to show the dynamics of the vegetation growth in middle Congo\hl{, as shown }in Figure \ref{fig08}. The NDVI is computed for each pixel, with brighter green hues indicating more dense and healthy plant canopy\hl{, }and contrariwise. 

Apparently, the combination of a \hl{higher} reflectance in NIR and \hl{a} lower reflectance in\hl{ }Red bands\hl{ }results in \hl{a} higher NDVI values \hl{in general, that is, }around 1\hl{, which represents} spectral signature of vegetation. Such values refer to the healthy plant coverage, typical for the tropical rainforest and other regions of dense canopy, while bank areas of Congo river, bare land, sparsely vegetated areas and urban spaces near Bumba, Basoko and Kisangani have lower NDVI values\hl{, respectively. For instance, }dark brown to black areas of the NDVI with values around zero are non-vegetated areas \hl{and water areas have negative values below sero}. Middle values indicate various types of shrubland, riparian plant communities along the river Congo and tributaries. Thus, the general approach is that in the NDVI \hl{the} scale \hl{is} ranging from \hl{-1} to 1 \hl{with positive values from 0 to 1 indicating the terrain areas, while water surfaces have values below 0. Higher NDVI} values (ca. from 0.7 to 1.0) are indicated by the \hl{healthy }vegetation, while lower values (from 0.0 to 0.3) represent the non-vegetated areas\hl{, correspondingly}. 
...
\hl{Figure} \ref{fig07} \hl{shows changes in} land cover types\hl{ in Bumba region }for 9-year time\hl{ }according to the\hl{ k-means }classification \hl{by R}.
...
\hl{Pairwise} examination of the classified images allows to examine the changes in forest structure \hl{which are caused by} the anthropogenic activities\hl{, including }urban growth\hl{, }and environmental drivers initiated by climate change. \hl{To this end, we present the results of the classification of the }three target regions \hl{showing changes in} tropical moist forest along the flow of Congo River\hl{, as shown in }Figure \ref{fig05}, Figure \ref{fig06} and Figure \ref{fig07} for Bumba, Basoko and Kisangani, respectively. 
...
\hl{Classes} of \hl{the} land cover types were defined as follows: Swamp forest; Humid tropical forest; Secondary forest; Urban areas and impervious surfaces; Inundated grassland; Deciduous forest; Shrubland and grassland; Cropland and mosaic forest; Savannah with sparse trees; Water bodies. The model was done by comparing the results of \hl{the} vegetation types classified \hl{based} on the Landsat imagery in 2013 and\hl{ }2022. The additional image was selected for the Kisingani for\hl{ }2015 due to the cloudiness of the image in 2013.\hl{ }As demonstrated on the resulting plots, the pixel-based models use 10 land-cover categories. The integration of \hl{the} two Landsat OLI/TIRS images for land cover mapping by R language is a challenging and promising approach \hl{which} can be recommended for\hl{ }future similar studies on rainforest mapping\hl{. }The final outputs as classified maps contained 10 land cover classes recorded from the 30 m raster grids equivalent to the resolution of the \hl{initial} input Landsat OLI/TIRS data. 

\hl{Land} cover classification using R approach \hl{for processing remote sensing data} by RStoolbox \hl{is a promising and effective approach}. However, \hl{a} pixel-based \hl{data processing} of the \hl{satellite images} results in the grid structure of the landscape with resolution corresponding to the initial data. For more details data, Sentinel scenes can be considered \hl{as well }for \hl{a} more fine-resolution classification. The continued increase of \hl{the} volume of spatial Earth observation data from the satellite images and novel directions in image analysis techniques using programming tools encourage the development of object based approaches. \hl{Therefore}, \hl{technical} variations in\hl{ }the classification process can be omitted by applying the object based image analysis for detecting \hl{the} landscape structure\hl{. This }can be considered as a further development of the R programming methods.
...
\hl{Three examples} of the NDVI computed from the Red/NIR bands of the Landsat OLI-TIRS images have heterogeneous landscapes and vegetation coverage,\hl{ }Figure \ref{fig08}. 
...
\hl{DEM} & \hl{Digital Elevation Model} \\
DN & Digital Number \\
...
\hl{L1TP} & {Level 1 Terrain Precision (Corrected)} \\
